...
\begin{abstract}
Pixel-level annotations for medical image segmentation are expensive because they require substantial effort from clinical experts. It is therefore important to learn reliable segmentation models from only a few labeled images together with a much larger unlabeled set. Existing semi-supervised methods commonly generate pseudo labels for each unlabeled image independently, while cross-image approaches often transfer supervision from a labeled image to a target in a single step. Such supervision can become unreliable when the labeled source and target differ greatly in lesion shape, size, or appearance. We propose RouteSAM, a semantic-routing framework that views unlabeled images not only as training targets, but also as intermediate steps connecting labeled and difficult target images. Under a fixed annotation budget, RouteSAM first selects a small set of representative images as labeled Anchors. For each Target, it then retrieves semantically related unlabeled Bridge images and orders them into an Anchor--Bridge--Target route. SAM3 propagates the Anchor mask progressively along this route, decomposing a difficult cross-image transfer into a sequence of smoother local transitions and producing multiple candidate masks for the Target. Because errors may still accumulate during propagation, a task-specific segmentation model is used to evaluate and learn from reliable candidates, while its domain knowledge improves a subsequent round of SAM3 propagation. Semantic routing is required only during training: the final model segments each test image directly, without retrieving reference images, constructing routes, or requiring manual prompts. Experiments on Kvasir-SEG, BUSI, and ISIC2018 under an approximately 1\% annotation budget show that RouteSAM \textbf{[final quantitative results to be inserted]}.
\end{abstract}
